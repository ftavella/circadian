...
# %% auto 0
__all__ = ['generate_activity_from_light', 'generate_correlated_activity_from_light']
...
# %% ../nbs/api/10_synthetic_data.ipynb 6
def generate_correlated_activity_from_light(
    time: np.ndarray, # time points.
    light: LightSchedule, # light schedule.
    mu_l: float=5.0, # mean of low activity levels.
    mu_h: float=25.0, # mean of high activity levels.
    ...
    sigma_h: float=30.0, # std of high activity levels.
    sigma_s: float=2.0, # std of activity during sleep.
    rho_l: float=0.5, # correlation coefficient between future and past low activity levels. Value must be between -1 and 1.
    rho_h: float=0.5, # correlation coefficient between future and past high activity levels. Value must be between -1 and 1.
    rho_s: float=0.5, # correlation coefficient between future and past sleep activity levels. Value must be between -1 and 1.
    active_level: float=0.5, # person's activity level from 0 to 1. Represents the probability of choosing from the high or low activity distributions at any point in time. Values close to 1 increase the proportion of high activity values.
) -> np.ndarray:
    "Generates a correlated 'activity' schedule from an input light schedule using an autoregressive model of lag 1. The activity schedule produced is based on the light schedule."
    # input validation
    # TODO
    ...
    if light(time[0]) == 0:
        activity[0] = np.abs(np.random.normal(mu_s, sigma_s))
    elif light(time[0]) > 0:
        high_activity = np.random.uniform() <= active_level
        if high_activity == True:
            activity[0] = np.abs(np.random.normal(mu_h, sigma_h))
        elif high_activity == False:
            activity[0] = np.abs(np.random.normal(mu_l, sigma_l))
    
    # noise standard deviation for each activity level
    sigma_error_l = np.sqrt((1 - rho_l**2) * sigma_l**2)
    sigma_error_h = np.sqrt((1 - rho_h**2) * sigma_h**2)
    sigma_error_s = np.sqrt((1 - rho_s**2) * sigma_s**2)
    
    # generate the rest of the values using an autoregressive model of lag 1
    for t in range(time.shape[0]):
        if light(time[t]) == 0:
            error_sleep_t = np.random.normal(0, sigma_error_s)
            activity[t] = mu_s + rho_s * (activity[t-1] - mu_s) + error_sleep_t
        elif light(time[t]) > 0:
            high_activity = np.random.uniform() <= active_level
            if high_activity == True:
                error_high_t = np.random.normal(0, sigma_error_h)
                activity[t] = mu_h + rho_h * (activity[t-1] - mu_h) + error_high_t
            elif high_activity == False:
                error_low_t = np.random.normal(0, sigma_error_l)
                activity[t] = mu_l + rho_l * (activity[t-1] - mu_l) + error_low_t
